...
function eval_grads!(Dy, backend::AbstractAutoDiffBackend, func!, x, p, f_is_in_place::Val{true})
    return error("`eval_grads!` not implemented for backend of type $(typeof(backend)) and funtions that are in-place.")
end

function eval_grads!(Dy, backend::AbstractAutoDiffBackend, func, x, p, f_is_in_place::Val{false})
    return error("`eval_grads!` not implemented for backend of type $(typeof(backend)) and funtions that are out-of-place.")
end

function eval_hessians!(H, backend::AbstractAutoDiffBackend, func, x, p, f_is_in_place::Union{Val{true}, Val{false}})
    return error("`eval_hessians!` not implemented for backend of type $(typeof(backend)).")
end

function eval_op_and_grads!(y, Dy, backend::AbstractAutoDiffBackend, func, x, p, f_is_in_place::Val{true})
...
function eval_op_and_grads!(y, Dy, backend::AbstractAutoDiffBackend, func, x, p, f_is_in_place::Val{false})
    @warn ("`eval_op_and_grads!` not implemented for backend of type $(typeof(backend)).")
    y .= func(x, p)
    return eval_grads!(Dy, backend, func, x, p, f_is_in_place) 
end
